...
Axelrod adopted a simple but elegant way for encoding strategies, and then used a single-objective evolutionary algorithm to obtain optimal strategies. His encoding scheme remained a standard way of handling the IPD problem and is described below. We also adopt a similar encoding scheme to compute optimal strategies for the game. Axelrod's method had the following features,
	...
	\item \textbf{Always Defect:} Defects on every move3) Tit for Tat : Cooperates on the first move, then simply copies the opponent's last move.
	\item \textbf{Suspicious Tit for Tat:} Same as Tit for Tat, except that it defects on the first move
	\item \textbf{Spiteful:} Cooperates, until the opponent defects, and thereafter always defects.
	\item \textbf{Random:} Makes a random move.
	\item \textbf{Cyclic CD:} Plays C, D periodically.
...
cooperated is greater than or equal to the number of times it has defected, else it defects.
	...
	The tournament setup is inspired by Axelrod'd initial study. It consists of a round robin tournament amongst all the strategies where each game is played for 200 turns. The whole tournament is repeated a few times (10 times in our experiments) to lessen the effect of randomness.

	\section{Results}
	
	\subsection{Plots for the Evolutionary Algorithm}

	\subsubsection{Single Objective Genetic Algorithm (Max-Avg Fitness)}	

	\textbf{Maximum Fitness (Generation wise)}\\
	\textbf{Average Fitness (Generation wise)}

	\subsubsection{Multi Objective Genetic Algorithm (Max-Avg Fitness)}
		
	\textbf{Maximum Fitness (Generation wise)}\\
	\textbf{Average Fitness (Generation wise)}\\
	\textbf{Plots for final population}
	...
	\subsection{Similarities between Optimal strategies}
